...
const ProductsPage: React.FC = () => {
  ...
  const [categoriesTree, setCategoriesTree] = useState<TreeCategory[]>([]); // new
  const [selectedPath, setSelectedPath] = useState<string[]>([]); // names path
  ...
  useEffect(() => {
    const loadAll = async () => {
      try {
        ...
        const normalize = (n:any):TreeCategory => ({ id: n._id || n.id, name: n.name, children: (n.children||[]).map((c:any)=>normalize(c)) });
        setCategoriesTree(Array.isArray(treeRes.data) ? treeRes.data.map(normalize) : []);
      } catch (e:any) {
  ...
  // derive categories at current depth
  ...
  // filter products by selectedPath hierarchy (match prefix of product path)
  const productsFilteredByPath = useMemo(() => {
    if (selectedPath.length === 0) return products; // show all if nothing selected
  ...
  const filtered = useMemo(() => {
    ...
    if (query.trim()) {
      ...
      data = data.filter(p =>
        ...
        (Array.isArray(p.path) ? p.path.join(' > ').toLowerCase().includes(q) : false)
      );
    }
    if (categoryFilter) {
      data = data.filter(p => Array.isArray(p.path) && p.path[0] === categoryFilter);
    }
    switch (sort) {
      ...
      <div className="mb-8 flex flex-col gap-6">
        <div className="flex items-end flex-wrap gap-4">
          <div className="flex flex-col">
            ...
            <input
              ...
              className="px-3 py-2 rounded border border-gray-300 bg-white focus:outline-none focus:ring focus:ring-blue-500/30 min-w-[260px]"
            />
          </div>
          <div className="flex flex-col">
            ...
            <select
              ...
              className="px-3 py-2 rounded border border-gray-300 bg-white focus:outline-none focus:ring focus:ring-blue-500/30"
            >
          ...
          <div className="flex flex-col">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`px-3 py-2 rounded border text-sm ${view==='grid' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-gray-300'}`}
              >Grid</button>
              <button
                onClick={() => setView('list')}
                className={`px-3 py-2 rounded border text-sm ${view==='list' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white border-gray-300'}`}
              >List</button>
            </div>
          </div>
        </div>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button onClick={() => {
              setLoading(true);
              api.get('/products').then(r=> setProducts(r.data)).catch(e=> setError(e?.message||'Failed')).finally(()=> setLoading(false));
            }} className="text-sm px-3 py-2 rounded border bg-white hover:bg-gray-50">Refresh</button>
            <a href="/create-products" className="text-sm font-medium text-blue-600 hover:underline">+ Create / Manage Structure</a>
          </div>
        </div>
      </div>
      ...
      <div className="mb-4 flex flex-wrap gap-2 items-center">
        ...
        {selectedPath.map((name, idx) => (
          <button
            ...
            className="text-sm px-2 py-1 rounded bg-blue-50 hover:bg-blue-100 text-blue-700"
          >{name}</button>
        ))}
        {selectedPath.length > 0 && (
          <button onClick={goBackOne} className="text-xs px-2 py-1 rounded border bg-white hover:bg-gray-50">Back</button>
        )}
        {selectedPath.length > 0 && (
          <button onClick={resetPath} className="text-xs px-2 py-1 rounded border bg-white hover:bg-gray-50">Reset</button>
        )}
      </div>
          ...
          {currentLevelCategories.length > 0 && (
            <ul className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
              {currentLevelCategories.map(cat => (
                <li key={cat.id}>
                  <button
                    onClick={() => setSelectedPath(p => [...p, cat.name])}
                    className="w-full text-left p-4 rounded border bg-white hover:shadow-sm transition"
                  >
          ...
          {filtered.map(p => (
            <li key={p.id} className="group rounded-lg border bg-white shadow-sm hover:shadow-md transition overflow-hidden">
              ...
              <div className="p-4 flex flex-col gap-2">
                ...
                <button className="mt-1 w-full text-xs font-medium px-3 py-2 rounded bg-blue-600 text-white hover:bg-blue-500">View / Edit</button>
              </div>
            </li>
          ))}
        </ul>
            ...
            <tbody>
              {filtered.map(p => (
                <tr key={p.id} className="border-t last:border-b">
                  ...
                  <td className="px-4 py-2 text-right"><button className="text-blue-600 hover:underline text-xs font-medium">View / Edit</button></td>
                </tr>
              ))}
            </tbody>
          </table>
